...
PACKAGE_ROOT = Path(os.environ.get("PACKAGE_ROOT", "/opt/cloud-deploy-candidates/matrix-v1"))
...
async def matrix_html_no_cache(request: Request, call_next):
    ...
    if request.url.path in {"/Matrix", "/Matrix/", "/Matrix/v1", "/Matrix/v2", "/Matrix/notifications"}:
...
def notification_window_start(reference: datetime | None = None) -> datetime:
    """Matrix notifications reset at 06:00 IST, not midnight."""
...
def last_closed_trade_for_instrument(instrument_id: Any) -> dict[str, Any]:
    text_id = str(instrument_id or "").strip()
    if not text_id:
        return {}
    state_path = PORTABLE_STATE_ROOT / "instruments" / safe_state_name(text_id) / "model_state.json"
...
def matrix_instruments_with_universe_placeholders(state: dict[str, Any]) -> dict[str, Any]:
    ...
    for entry in portable_universe_entries():
        ...
        instruments[symbol] = {
            ...
            "placeholder_reason": "no_v2_selected_leg_ledger_yet",
...
@app.get("/Matrix/v1", response_class=HTMLResponse)
...
@app.get("/Matrix/v2", response_class=HTMLResponse)
...
@app.get("/Matrix/notifications", response_class=HTMLResponse)
...
@app.get("/api/matrix/v1/status")
def matrix_status() -> JSONResponse:
    return matrix_status_response()


@app.get("/api/matrix/v2/status")
def matrix_status_v2() -> JSONResponse:
    return matrix_status_response()


@app.get("/api/matrix/v2/notifications/today")
...
<head>
  ...
  <title>Matrix</title>
  <script>
    document.documentElement.dataset.theme = localStorage.getItem("matrix-theme") || "dark";
  </script>
  <style>
    :root {
      color-scheme: dark;
...
<body>
  ...
  <script>
    ...
    async function refresh() {
      try {
        const response = await fetch("/api/matrix/v1/status", {cache:"no-store"});
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Matrix Alerts | Viveka</title>
  <style>
    :root {
      --bg:#212121;
      --surface:#2f2f2f;
...
<body>
  <header class="shell-header">
    <div class="topbar">
      <div class="brand">
        <a class="back-link" href="/Matrix/" aria-label="Back to Matrix">
          <svg viewBox="0 0 24 24" aria-hidden="true"><path d="m15 18-6-6 6-6"></path></svg>
        </a>
        <div class="brand-copy">
          <strong>Matrix Alerts</strong>
        </div>
      </div>
    </div>
  </header>
  ...
  <script>
    const LAST_OPENED_KEY = "matrix-v2-notifications-last-opened-ms";
    const LAST_OPENED_WINDOW_KEY = "matrix-v2-notifications-last-opened-window";
    const NOTIFICATION_ENABLED_KEY = "matrix-v2-notifications";
    const SOUND_ENABLED_KEY = "matrix-v2-sound";
    ...
    function emitSampleConfirmation() {
      ...
      if (notificationSupported() && Notification.permission === "granted") {
        try {
          new Notification("Matrix notifications enabled", {
            body: "You will receive Matrix alerts here.",
            tag: "matrix-notifications-enabled-sample"
          });
        } catch (error) {}
      }
    }
    ...
    async function loadNotifications() {
      try {
        const response = await fetch("/api/matrix/v2/notifications/today", {cache:"no-store"});
        ...
        document.getElementById("notificationCards").innerHTML = `<div class="empty">Unable to load Matrix notifications</div>`;
      }
    }
    ...
    document.documentElement.dataset.theme = localStorage.getItem("matrix-v2-theme") || "dark";
    updateToggle();
    loadNotifications();
...
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>Matrix | Institutional Trading System</title>
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap" rel="stylesheet">
  <script>
    const storedMatrixTheme = localStorage.getItem("matrix-v2-theme");
    const matrixThemeUserSet = localStorage.getItem("matrix-v2-theme-user-set") === "true";
...
<body>
  ...
  <header class="shell-header">
    <div class="topbar">
      ...
      <a class="header-notifications" id="headerNotifications" href="/Matrix/notifications" aria-label="Open notifications">
        <svg viewBox="0 0 24 24" aria-hidden="true">
      ...
      <div class="top-actions">
        ...
        <div class="sidebar-account guest" id="sidebarAccount">
          <div class="account-panel" id="accountPanel">
            <a href="/Matrix/" class="account-action">Profile</a>
            <a href="/Matrix/notifications" class="account-action">Settings</a>
  ...
  <script>
    ...
    let notificationsEnabled = localStorage.getItem("matrix-v2-notifications") === "true";
    let soundEnabled = localStorage.getItem("matrix-v2-sound") === "true" || notificationsEnabled;
    let mobileNotificationPromptAttempted = false;
    let watchlistFilterActive = localStorage.getItem("matrix-v2-watchlist-filter") === "true";
    ...
    const HIGHLIGHT_STORAGE_KEY = "matrix-v2-signal-highlights";
    const NOTIFICATION_LAST_OPENED_KEY = "matrix-v2-notifications-last-opened-ms";
    const filters = ["All", "Bullish", "Bearish", "Neutral"];
    const statusRank = { Bullish: 0, Bearish: 1, Neutral: 2 };
    const missing = value => value === null || value === undefined || value === "" || value === "NA";
    const safe = (value, fallback = "Not available") => missing(value) ? fallback : String(value);
    ...
    function loadWatchlist() {
      try {
        const raw = JSON.parse(localStorage.getItem("matrix-v2-watchlist") || "[]");
    ...
    function saveWatchlist() {
      localStorage.setItem("matrix-v2-watchlist", JSON.stringify([...watchlistIds].sort()));
      localStorage.setItem("matrix-v2-watchlist-filter", watchlistFilterActive ? "true" : "false");
    }
    ...
    function loadSignalHighlights() {
      try {
        const raw = JSON.parse(localStorage.getItem("matrix-v2-signal-highlights") || "{}");
    ...
    async function promptMobileNotificationsOnOpen(force = false) {
      ...
      soundEnabled = true;
      localStorage.setItem("matrix-v2-sound", "true");
      if (!notificationSupported()) {
        notificationsEnabled = false;
        localStorage.setItem("matrix-v2-notifications", "false");
        updateNotifyButton();
        return;
      }
      if (Notification.permission === "default") {
      ...
      localStorage.setItem("matrix-v2-notifications", notificationsEnabled ? "true" : "false");
      updateNotifyButton();
    }
    ...
    async function requestNotifications() {
      ...
      if ((notificationSupported() && Notification.permission === "granted" && notificationsEnabled) || (!notificationSupported() && soundEnabled)) {
        ...
        soundEnabled = false;
        localStorage.setItem("matrix-v2-notifications", "false");
        localStorage.setItem("matrix-v2-sound", "false");
        updateNotifyButton();
        return;
      }
      soundEnabled = true;
      ...
      if (notificationsEnabled) {
        try {
          new Notification("Matrix notifications enabled", {
            ...
            tag:"matrix-v2-notifications-enabled"
          });
        } catch (error) {}
      }
    }
    ...
    function notifySignal(row, options = {}) {
      ...
      try {
        new Notification(parts.title, {
          ...
          vibrate:[80, 45, 80],
          silent:false,
          tag: `matrix-v2-${rowId(row)}-${safe(row.last_trade_event_id || row.trigger_time_ist, Date.now())}`,
          renotify:true
        });
      } catch (error) {}
    }
    async function emitSmokeAlert() {
      ...
      if (notificationSupported() && Notification.permission === "granted") {
        ...
        soundEnabled = true;
        localStorage.setItem("matrix-v2-notifications", "true");
        localStorage.setItem("matrix-v2-sound", "true");
        updateNotifyButton();
    ...
    function applyTheme(theme, userSelected = false) {
      document.documentElement.dataset.theme = theme;
      localStorage.setItem("matrix-v2-theme", theme);
      if (userSelected) localStorage.setItem("matrix-v2-theme-user-set", "true");
    ...
    function initAccountDrawer() {
      const storedName = String(localStorage.getItem("matrix-v2-user-name") || "").trim();
      ...
      document.getElementById("accountRole").textContent = loggedIn ? "Matrix account" : "Not signed in";
    ...
    async function refresh() {
      ...
      try {
        const response = await fetch("/api/matrix/v2/status", {cache:"no-store"});
    ...
    function startMatrixEventStream() {
      ...
      let lastStreamKey = "";
      const connect = () => {
        source = new EventSource("/api/matrix/v2/stream");
